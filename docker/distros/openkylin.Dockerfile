...
RUN apt-get update && apt-get install -y llvm-17-tools coreutils util-linux netcat yq grep procps bash sudo git python3.12-venv wget build-essential zstd locales && apt-get clean
