...
RUN apt-get update && apt-get install -y llvm-14-tools coreutils util-linux tcping yq grep procps bash sudo git python3.11-venv wget build-essential libssl-dev zstd locales && apt-get clean
