...
RUN dnf install -y python3-lit llvm18 coreutils util-linux grep procps bash sudo git wget make zstd openssl jq glibc-locale-source python3-pip iputils nmap-ncat
RUN pip install yq
